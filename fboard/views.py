...
def f_view(request,nowpage,f_No):
    ...
    for i,board in enumerate(allboard):
        ...
        if board.f_No==f_No:
            ...
            print(afterboard)
            print(f_No)
            context={'f_board':f_board,"afterboard":afterboard,"beforeboard":beforeboard,'nowpage':nowpage }
            return render(request,'comment_view.html',context)
        
        # else:
        #     afterboard=allboard[f_No]
        #     context={'f_board':f_board,"afterboard":afterboard,'nowpage':nowpage }
        #     return render(request,'comment_view.html',context)
    ...
    #     prevboard=None
    #     try:
    #         nextboard=f_board.objects.get(f_No=f_No+1)
    #     except f_board.DoesNotExist:
    #         nextboard=None
    
    # else:
    #     prevboard=f_board.objects.get(f_No=f_No-1)
    #     try:
    #         nextboard=f_board.objects.get(f_No=f_No+1)
    #     except f_board.DoesNotExist:
    #         nextboard=None
    # ,'nextboard':nextboard,'prevboard':prevboard
...
def f_reply(request,nowpage,f_No):
    if request.method=='GET':
        ...
        qs=Fboard(f_writer=writer,f_Title=f_Title,f_Detail=f_Detail,f_group=f_group,f_step=f_step+1,f_indent=f_indent+1) # F()디비속의 값 가져오기
        qs.save()
        return redirect('fboard:f_list',nowpage)
