$(function () {
	...
	$.fn.extend({
		searchStyle: function (options) {
			this.each(function () {
				...
				$(this)
					.after(
						'<span class="searchStyleSelectBox"><span class="searchStyleSelectBoxInner">' +
				...
				var selectBoxWidth =
					parseInt($(this).width()) - parseInt(selectBoxSpan.css("padding-left"));
	...
	$(".sum").text(sum);
	if (wishnum < 3) {
		$(".wish .list").css("height", "auto");
	} else {
		$(".wish .list").css("height", "264px");
	}
	...
	$(".wishRight").click(function () {
		...
		if (!$ani.is(":animated")) {
			...
			if (chkTop == wbig) {
				...
				$(".wish ul").stop().animate({ top: chkTop }, 300);
			}
		}
	});
	...
	$(".wishLeft").click(function () {
		...
		if (!$ani.is(":animated")) {
			...
			if (chkbom == 0) {
				...
				chkbom = chkbom + 264;
				$(".wish ul").stop().animate({ top: chkbom }, 300);
			}
		}
	});
