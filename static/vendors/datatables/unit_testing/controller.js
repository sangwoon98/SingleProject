...
function fnTestStart(sTestInfo) {
	...
	gtoTest = setTimeout(function () {
		fnMessage(
			'<span class="error">WARNING - test script stalled. Likely a JS error</span>'
		);
		gbStop = true;
	}, 3000);
}
...
function fnEnd(bSuccess) {
	...
	var sTime =
		"<br>This test run took " +
		parseInt((iEndTime - giStartTime) / 1000, 10) +
		" second(s) to complete.";

	if (bSuccess) {
		$("#test_running").html(
			"Tests complete. " + giTotalTestCount + " tests were run." + sTime
		);
	} else {
		$("#test_running").html(
			"Unit tests failed at test " + giTotalTestCount + "." + sTime
		);
	}
}
