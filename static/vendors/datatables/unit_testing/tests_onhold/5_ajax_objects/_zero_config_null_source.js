...
oTest.fnStart(
	"Sanity checks for DataTables with data from JS - Null data source for last column"
);

oTest.fnTest("jQuery.dataTable function", null, function () {
	return typeof jQuery().dataTable == "function";
});

oTest.fnTest("jQuery.dataTableSettings storage array", null, function () {
	return typeof jQuery().dataTableSettings == "object";
});

oTest.fnTest("jQuery.dataTableExt plugin object", null, function () {
	return typeof jQuery().dataTableExt == "object";
});

$(document).ready(function () {
	var oInit = {
		aoColumns: [null, null, null, null, { mData: null }],
		aaData: gaaData,
	};
	$("#example").dataTable(oInit);

	/* Basic checks */
	oTest.fnWaitTest("Length changing div exists", null, function () {
		return document.getElementById("example_length") != null;
	});

	oTest.fnTest("Filtering div exists", null, function () {
		return document.getElementById("example_filter") != null;
	});

	oTest.fnTest("Information div exists", null, function () {
		return document.getElementById("example_info") != null;
	});

	oTest.fnTest("Pagination div exists", null, function () {
		return document.getElementById("example_paginate") != null;
	});

	oTest.fnTest("Processing div is off by default", null, function () {
		return document.getElementById("example_processing") == null;
	});

	oTest.fnWaitTest("10 rows shown on the first page", null, function () {
		return $("#example tbody tr").length == 10;
	});

	oTest.fnTest("Initial sort occured", null, function () {
		return $("#example tbody td:eq(0)").html() == "Gecko";
	});

	/* Need to use the WaitTest for sorting due to the setTimeout datatables uses */
	oTest.fnTest("Data in last column is empty", null, function () {
	...
	oTest.fnTest("Data in last column is still empty", null, function () {
		return $("#example tbody td:eq(4)").html() == "";
	});
	 ...
	 * Information element
	 */
	oTest.fnTest("Information on zero config", null, function () {
		return (
	...
	oTest.fnTest("Filter 'W' - info", null, function () {
		return (
			document.getElementById("example_info").innerHTML ==
	...
	oTest.fnTest(
		...
		function () {
			return (
				$("#example tbody tr:eq(0) td:eq(1)").html() == "AOL browser (AOL desktop)"
			);
		}
	);

	oTest.fnTest("Filter 'Win' - sorting column 1 info", null, function () {
		return (
			document.getElementById("example_info").innerHTML ==
			"Showing 1 to 10 of 31 entries (filtered from 57 total entries)"
		);
	...
	oTest.fnTest("Filter 'nothinghere' - info", null, function () {
		return (
			document.getElementById("example_info").innerHTML ==
