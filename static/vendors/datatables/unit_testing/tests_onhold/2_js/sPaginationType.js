...
$(document).ready(function () {
	...
	oTest.fnTest("Check two button paging is the default", null, function () {
		return oSettings.sPaginationType == "two_button";
	});

	oTest.fnTest("Check class is applied", null, function () {
		return $("#example_paginate").hasClass("paging_two_button");
	});

	oTest.fnTest("Two A elements are in the wrapper", null, function () {
		return $("#example_paginate a").length == 2;
	});

	oTest.fnTest("We have the previous button", null, function () {
		return document.getElementById("example_previous");
	});

	oTest.fnTest("We have the next button", null, function () {
		return document.getElementById("example_next");
	});

	oTest.fnTest("Previous button is disabled", null, function () {
		return $("#example_previous").hasClass("paginate_disabled_previous");
	});

	oTest.fnTest("Next button is enabled", null, function () {
	...
	oTest.fnTest("Check full numbers class is applied", null, function () {
