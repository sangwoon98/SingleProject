...
(function (a, b) {
		...
		p.extend({
			queue: function (a, b, c) {
				...
				if (a)
					return (
						...
						(d = p._data(a, b)),
						c &&
							(!d || p.isArray(c) ? (d = p._data(a, b, p.makeArray(c))) : d.push(c)),
						d || []
					);
			},
			dequeue: function (a, b) {
				...
				e === "inprogress" && ((e = c.shift()), d--),
					e &&
						(b === "fx" && c.unshift("inprogress"), delete f.stop, e.call(a, g, f)),
					!d && f && f.empty.fire();
			},
			...
			for (d = 0; d < u.length && !c.isPropagationStopped(); d++) {
				...
				for (
					...
					if (
						...
						(c.data = l.data),
							(c.handleObj = l),
							(g = ((p.event.special[l.origType] || {}).handle || l.handler).apply(
								i.elem,
								r
							)),
							g !== b &&
				...
				return b.first
					...
					: function (b, d, h) {
							if (!h) {
								...
								while ((b = b[e])) if (f || b.nodeType === 1) if (a(b, d, h)) return b;
					  };
			}
			function bj(a) {
				return a.length > 1
			...
			function bm(a) {
				var b,
					...
					m = [
						function (a, c, d) {
							return (
								(!g && (d || c !== l)) || ((b = c).nodeType ? j(a, c, d) : k(a, c, d))
							);
						},
					];
				for (; i < f; i++)
					...
					s.contains
						? function (a, b) {
								var c = a.nodeType === 9 ? a.documentElement : a,
									d = b && b.parentNode;
								return (
									a === d || !!(d && d.nodeType === 1 && c.contains && c.contains(d))
								);
						  }
							...
							CLASS: function (a) {
								...
								return (
									...
									function (a) {
										return b.test(
											a.className ||
												(typeof a.getAttribute !== n && a.getAttribute("class")) ||
												""
										);
									}
								);
	...
	p.fn.extend({
		...
		add: function (a, b) {
			var c =
					typeof a == "string" ? p(a, b) : p.makeArray(a && a.nodeType ? [a] : a),
		...
		p.each(
			...
			function (a, b) {
				p.fn[a] = function (c) {
					...
					if (
						(i == null || (i && i.nodeType === 11 && i.childNodes.length === 1)) &&
						h === 1
					)
	...
	(cj = ct.exec(ck.toLowerCase()) || []),
		...
		p.extend({
			...
			ajaxSetup: function (a, b) {
				return (
					b ? cB(a, p.ajaxSettings) : ((b = a), (a = p.ajaxSettings)), cB(a, b), a
				);
			},
			ajaxSettings: {
				url: ck,
	...
	p.ajaxSetup({
		...
		p.ajaxPrefilter("json jsonp", function (c, d, e) {
			...
			if (c.dataTypes[0] === "jsonp" || l || m)
				return (
					(f = c.jsonpCallback =
						p.isFunction(c.jsonpCallback) ? c.jsonpCallback() : c.jsonpCallback),
					(g = a[f]),
					l
