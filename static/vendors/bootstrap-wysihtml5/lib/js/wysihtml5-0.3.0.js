	...
	return {
		...
		supportsDataTransfer: function () {
			try {
				...
				return (
					isWebKit && (window.Clipboard || window.DataTransfer).prototype.getData
				);
			} catch (e) {
				return false;
			}
	...
	function resolveList(list) {
		...
		while ((listItem = list.firstChild)) {
			...
			while ((firstChild = listItem.firstChild)) {
				...
				shouldAppendLineBreak =
					isLastChild && !_isBlockElement(firstChild) && !_isLineBreak(firstChild);
	...
	wysihtml5.views.Composer.prototype.observe = function () {
		...
		dom.observe(element, pasteEvents, function (event) {
			...
			if (dataTransfer && browser.supportsDataTransfer()) {
				data =
					dataTransfer.getData("text/html") || dataTransfer.getData("text/plain");
			}
			if (data) {
				element.focus();
	...
	wysihtml5.Editor = wysihtml5.lang.Dispatcher.extend(
		/** @scope wysihtml5.Editor.prototype */ {
			constructor: function (textareaElement, config) {
				...
				if (
					!this._isCompatible ||
					(!this.config.supportTouchDevices && wysihtml5.browser.isTouchDevice())
				) {
					var that = this;
					setTimeout(function () {
