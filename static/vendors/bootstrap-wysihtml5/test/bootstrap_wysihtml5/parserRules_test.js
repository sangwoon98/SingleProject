if (wysihtml5.browser.supported()) {
	...
	for (c in colors) {
		colorText =
			colorText + '<span class="wysiwig-color-' + c + '">' + c + "</span>";
	}
	...
	module("wysihtml5.bootstrap.parserRules", {
		setup: function () {
			...
			for (c in this.colors) {
				this.colorText =
					this.colorText + '<span class="wysiwyg-color-' + c + '">' + c + "</span>";
			}
		},
	...
	asyncTest("check that he default parser rules work as expected", function () {
		...
		editor.observe("load", function () {
			equal(
				editor.parse("hello <b>foo</b>!").toLowerCase(),
				"hello <b>foo</b>!",
				"<b></b> tags are allowed"
			);

			equal(
				editor.parse(that.colorText).toLowerCase(),
				that.colorText,
				"classes used for font colors are allowed"
			);

			equal(
				editor.parse("hello <strong>foo</strong>!").toLowerCase(),
				"hello foo!",
				"<strong></strong> tags are stripped out"
			);

			equal(
				editor.parse("hello <faketag>foo</faketag>!").toLowerCase(),
				"hello foo!",
				"unrecognized tags are stripped out"
			);

			equal(
				editor
					.parse('hello <a href="http://seospammer.com">spam</a>!')
					.toLowerCase(),
			...
			equal(
				editor
					.parse(
						'standard internet image: <img width="foo" height="50px" src="cdn0.sbnation.com/imported_assets/155249/kitties.jpg">'
					)
					.toLowerCase(),
	...
	asyncTest(
		"check that parserRules passed in as options work properly",
		function () {
			expect(6);
			...
			editor.observe("load", function () {
				equal(
					editor.parse("hello <b>foo</b>!").toLowerCase(),
					"hello <b>foo</b>!",
					"<b></b> tags are still allowed"
				);

				equal(
					editor.parse('hello <span class="spanner">foo</span>!').toLowerCase(),
					'hello <span class="spanner">foo</span>!',
					"<span></span> tags are allowed"
				);

				equal(
					editor.parse(that.colorText).toLowerCase(),
					that.colorText,
					"classes used for font color are still allowed"
				);

				equal(
					editor.parse("hello <strong>foo</strong>!").toLowerCase(),
					"hello <strong>foo</strong>!",
					"<strong></strong> tags are allowed"
				);

				equal(
					editor.parse('hello <i class="bar">foo</i>!').toLowerCase(),
					'hello <i class="bar">foo</i>!',
					'<i class="bar"> survives'
				);

				equal(
					editor.parse("hello <em>foo</em>!").toLowerCase(),
					"hello <blink>foo</blink>!",
					"<em></em> tags are converted to <blink></blink>"
				);
