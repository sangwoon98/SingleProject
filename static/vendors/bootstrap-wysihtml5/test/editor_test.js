if (wysihtml5.browser.supported()) {
	module("wysihtml5.Editor", {
		...
		teardown: function () {
			var leftover;
			while (
				(leftover = document.querySelector(
					"iframe.wysihtml5-sandbox, input[name='_wysihtml5_mode']"
				))
			) {
	...
	asyncTest("Basic test", function () {
		...
		editor.observe("load", function () {
			...
			ok(
				wysihtml5.dom.hasClass(document.body, "wysihtml5-supported"),
				"<body> received correct class name"
			);
			...
			equal(
				...
				iframeElement,
				"Editor iframe is inserted after the textarea"
			);
			equal(
				composerElement.getAttribute("contentEditable"),
				"true",
				"Body element in iframe is editable"
			);
			equal(
				editor.textarea.element,
				textareaElement,
				"Textarea correctly available on editor instance"
			);
			equal(
				editor.composer.element,
				composerElement,
				"contentEditable element available on editor instance"
			);
			equal(
				wysihtml5.dom.getStyle("font-style").from(composerElement),
				"italic",
				"Correct font-style applied to editor element"
			);
			...
			if ("borderRadius" in document.createElement("div").style) {
				...
				ok(
					wysihtml5.dom
						.getStyle("border-top-right-radius")
						.from(iframeElement)
						.indexOf("2px") !== -1,
					"border-radius correctly copied"
				);
			}

			equal(
				composerElement.innerHTML.toLowerCase(),
				"hey tiff, what's up?",
				"Copied the initial textarea value to the editor"
			);
			ok(
				wysihtml5.dom.hasClass(composerElement, "wysihtml5-editor"),
				"Editor element has correct class name"
			);
	...
	asyncTest(
		"Check setting of name as class name on iframe and iframe's body",
		function () {
			expect(4);
			...
			editor.observe("load", function () {
				var iframeElement = that.getIframeElement(),
					...
					textareaElement = that.textareaElement;
				ok(
					wysihtml5.dom.hasClass(iframeElement, name),
					"iFrame has adopted name as className"
				);
				ok(
					wysihtml5.dom.hasClass(composerElement, name),
					"iFrame's body has adopted name as className"
				);
				ok(
					wysihtml5.dom.hasClass(composerElement, "death-star"),
					"iFrame's body has adopted the textarea className"
				);
				ok(
					!wysihtml5.dom.hasClass(textareaElement, name),
					"Textarea has not adopted name as className"
				);
				start();
			});
		}
	...
	asyncTest("Check whether customTemplates work", function () {
			...
			customTemplates: {
				...
				html: function (locale) {
					return (
						...
						"<a class='btn' data-wysihtml5-action='change_view' title='" +
						locale.html.edit +
						"'>HTML</a>" +
						"</div>" +
						"</li>"
		...
		editor.observe("load", function () {
			...
			equal(
				$("form .wysihtml5-toolbar .custom_image").length,
				1,
				"Custom image template works"
			);
			equal(
				$("form .wysihtml5-toolbar li .html-edit-button a").html(),
				"HTML",
				"Custom html template works"
			);
			start();
		});
	});
	...
	asyncTest("Check whether attributes are copied", function () {
		...
		editor.observe("load", function () {
			equal(
				that.getComposerElement().title,
				that.textareaElement.title,
				"Editor got attributes copied over from textarea"
			);
			start();
		});
	});
	...
	asyncTest("Check events", function () {
		...
		editor.observe("load", function () {
			...
			equal(
				wysihtml5.dom.getStyle("margin-top").from(iframeElement),
				"5px",
				":focus styles are correctly unset"
			);
	...
	asyncTest("Check sync (basic)", function () {
		...
		editor.observe("load", function () {
			...
			setTimeout(function () {
				equal(
					that.textareaElement.value.toLowerCase(),
					html.toLowerCase(),
					"Editor content got correctly copied over to original textarea"
				);
				start();
			}, 500);
		});
		...
		editor.observe("load", function () {
			...
			setTimeout(function () {
				...
				composerElement.innerHTML =
					'<font color="red">hey </font><strong>helen!</strong>';
				editor.fire("change_view", "textarea");
				equal(
					that.textareaElement.value.toLowerCase(),
					"hey <strong>helen!</strong>",
					"Editor got immediately copied over to textarea after switching the view"
				);
				...
				equal(
					composerElement.innerHTML.toLowerCase(),
					"hey <strong>richard!</strong>",
					"Textarea sanitized and copied over it's value to the editor after switch"
				);
				...
				equal(
					that.textareaElement.value.toLowerCase(),
					"hey <strong>timmay!</strong>",
					"Textarea gets the sanitized content of the editor onsubmit"
				);
				...
				setTimeout(function () {
					...
					setTimeout(function () {
						equal(
							wysihtml5.dom.getTextContent(composerElement),
							"",
							"Editor is empty after reset"
						);
						start();
					}, 100);
				}, 500);
	...
	asyncTest("Check placeholder", function () {
		...
		editor.observe("load", function () {
			...
			equal(
				wysihtml5.dom.getTextContent(composerElement),
				placeholderText,
				"Placeholder text correctly copied into textarea"
			);
			ok(
				wysihtml5.dom.hasClass(composerElement, "placeholder"),
				"Editor got 'placeholder' css class"
			);
			ok(
				editor.hasPlaceholderSet(),
				"'hasPlaceholderSet' returns correct value when placeholder is actually set"
			);
			editor.fire("focus:composer");
			equal(
				wysihtml5.dom.getTextContent(composerElement),
				"",
				"Editor is empty after focus"
			);
			ok(
				!wysihtml5.dom.hasClass(composerElement, "placeholder"),
				"Editor hasn't got 'placeholder' css class"
			);
			ok(
				!editor.hasPlaceholderSet(),
				"'hasPlaceholderSet' returns correct value when placeholder isn't actually set"
			);
			editor.fire("blur:composer");
			equal(
				wysihtml5.dom.getTextContent(composerElement),
				placeholderText,
				"Editor restored placeholder text after unfocus"
			);
			editor.fire("focus:composer");
			...
			equal(
				composerElement.innerHTML.toLowerCase(),
				html,
				"HTML hasn't been cleared even though the innerText and textContent properties indicate empty content."
			);
			ok(
				!wysihtml5.dom.hasClass(composerElement, "placeholder"),
				"Editor hasn't got 'placeholder' css class"
			...
			setTimeout(function () {
				...
				setTimeout(function () {
					equal(
						wysihtml5.dom.getTextContent(composerElement),
						placeholderText,
						"After form reset the editor has the placeholder as content"
					);
					start();
				}, 100);
			}, 500);
	...
	asyncTest("Check public api", function () {
		...
		editor.observe("load", function () {
			...
			ok(
				wysihtml5.dom.hasClass(document.body, "editor-is-supported"),
				"<body> received correct class name"
			);
			...
			equal(
				wysihtml5.dom.getTextContent(composerElement),
				"",
				"Editor empty after calling 'clear'"
			);
			ok(
				wysihtml5.dom.hasClass(composerElement, "editor"),
				"Composer element has correct class name"
			);
			...
			equal(
				composerElement.innerHTML.toLowerCase(),
				html,
				"Editor content correctly set after calling 'setValue'"
			);
			ok(
				!editor.isEmpty(),
				"'isEmpty' returns correct value when the composer element isn't actually empty"
			);
			...
			equal(
				value.toLowerCase(),
				html,
				"Editor content correctly returned after calling 'getValue'"
			);
			...
			ok(
				editor.isEmpty(),
				"'isEmpty' returns correct value when the composer element is actually empty"
			);

			equal(
				editor.parse("<p>foo</p>").toLowerCase(),
				"<div>foo</div>",
				"'parse' returns correct value"
			);
			...
			ok(
				!composerElement.getAttribute("contentEditable"),
				"When disabled the composer hasn't the contentEditable attribute"
			);
			equal(
				composerElement.getAttribute("disabled"),
				"disabled",
				'When disabled the composer has the disabled="disabled" attribute'
			);
			...
			equal(
				composerElement.getAttribute("contentEditable"),
				"true",
				"After enabling the editor the contentEditable property is true"
			);
			ok(
				!composerElement.getAttribute("disabled"),
				"After enabling the disabled attribute is unset"
			);
	...
	asyncTest(
		"Parser (default parser method with parserRules as object",
		function () {
			expect(2);
			...
			editor.observe("load", function () {
				equal(
					editor.config.parserRules,
					parserRules,
					"Parser rules correctly set on config object"
				);
				// Invoke parsing via second parameter of setValue()
				editor.setValue(input, true);
				equal(
					editor.getValue().toLowerCase(),
					output,
					"HTML got correctly parsed within setValue()"
				);
				start();
			});
		}
	...
	asyncTest(
		"Parser (custom parser method with parserRules as object",
		function () {
			expect(7);
			...
			$(this.textareaElement).wysihtml5("shallowExtend", {
				...
				parser: function (html, rules, context) {
					equal(
						html.toLowerCase(),
						input,
						"HTML passed into parser is equal to the one which just got inserted"
					);
					equal(
						rules,
						parserRules,
						"Rules passed into parser are equal to those given to the editor"
					);
					equal(
						context,
						that.getIframeElement().contentWindow.document,
						"Context passed into parser is equal the document object of the editor's iframe"
					);
					return html.replace(/\<script\>.*?\<\/script\>/gi, "");
				},
			});
	...
	asyncTest(
		"Inserting an element which causes the textContent/innerText of the contentEditable element to be empty works correctly",
		function () {
			expect(2);
			...
			editor.observe("load", function () {
				...
				setTimeout(function () {
					equal(
						composerElement.innerHTML.toLowerCase(),
						html,
						"Composer still has correct content"
					);
					equal(
						textareaElement.value.toLowerCase(),
						html,
						"Textarea got correct value"
					);
					start();
				}, 500);
			});
	...
	asyncTest("Check for stylesheets", function () {
		...
		editor.observe("load", function () {
			...
			equal(
				linkElements.length,
				2,
				"Correct amount of stylesheets inserted into the dom tree"
			);
