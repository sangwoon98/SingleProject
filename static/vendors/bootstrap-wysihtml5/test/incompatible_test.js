...
asyncTest("Basic test", function () {
	...
	editor.observe("load", function () {
		...
		ok(
			...
			"<body> didn't receive the 'wysihtml5-supported' class"
		);
		ok(
			!editor.isCompatible(),
			"isCompatible returns false when rich text editing is not correctly supported in the current browser"
		);
		...
		equal(
			...
			oldIframesLength,
			"No hidden field has been inserted into the dom"
		);
		equal(
		...
		editor.observe("focus:composer", function () {
			ok(
				false,
				"Specific 'focus:composer' event fired, and that's wrong, there shouldn't be a composer element/view"
			);
		});

		QUnit.triggerEvent(
			that.textareaElement,
			wysihtml5.browser.supportsEvent("focusin") ? "focusin" : "focus"
		);
