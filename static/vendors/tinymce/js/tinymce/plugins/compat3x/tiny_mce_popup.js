...
tinyMCEPopup = {
	...
	init: function () {
		...
		if (settings.popup_css !== false) {
			if (settings.popup_css) {
				...
				settings.popup_css = t.baseURI.toAbsolute(
					"themes/" + settings.theme + "/skins/" + settings.skin + "/dialog.css"
				);
			}
		}
		...
		if (settings.popup_css_add) {
			settings.popup_css +=
				"," + t.documentBaseURI.toAbsolute(settings.popup_css_add);
		}
	...
	getWin: function () {
		...
		return (
			(!window.frameElement && window.dialogArguments) || opener || parent || top
		);
	},
