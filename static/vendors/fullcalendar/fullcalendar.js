		...
		function renderSlotSegs(segs, modifiedEventId) {
			...
			for (i = 0; i < segCnt; i++) {
				...
				if ((eventElement = seg.element)) {
					...
					seg.vsides =
						val === undefined ? (vsideCache[key] = vsides(eventElement, true)) : val;
					val = hsideCache[key];
					seg.hsides =
						val === undefined ? (hsideCache[key] = hsides(eventElement, true)) : val;
			...
			eventElement.resizable({
				...
				resize: function (ev, ui) {
					...
					slotDelta = Math.round(
						(Math.max(slotHeight, eventElement.height()) - ui.originalSize.height) /
							slotHeight
					);
			...
			handle.mousedown(function (ev) {
				...
				hoverListener.start(function (cell, origCell) {
					if (cell) {
						...
						dayDelta =
							r * 7 + c * dis + dit - (origCell.row * 7 + origCell.col * dis + dit);
