...
(function (m, ma) {
			...
			if (!n || k != n.name) {
				...
				if ((n = Y[k])) n.element.show();
					...
					n = Y[k] = new Ja[k](
						(Z = s =
							m(
								"<div class='fc-view fc-view-" + k + "' style='position:absolute'/>"
							).appendTo(E)),
						X
					);
	...
	function Rb(a, b, e) {
		...
		function g() {
			...
			x +=
				"<td class='fc-agenda-gutter " + O + "'>&nbsp;</td></tr></tbody></table>";
			v = m(x).appendTo(a);
			F = v.find("thead");
			...
			if (i("allDaySlot")) {
				ia = m("<div style='position:absolute;z-index:8;top:0;left:0'/>").appendTo(
					ja
				);
				x =
			...
			if (h.which == 1 && i("selectable")) {
				...
				Ra.start(function (x, A) {
					...
					if (x && x.col == A.col && !pa(x)) {
						A = na(A);
						x = na(x);
						O = [A, xa(N(A), i("slotMinutes")), x, xa(N(x), i("slotMinutes"))].sort(
							Gb
						);
						c(O[0], O[3]);
					} else O = null;
									...
									if (v) {
										if (M) {
											...
											Eb(
												...
												ia *
													Math.round(
														(o.end ? (o.end - o.start) / wc : u("defaultEventMinutes")) /
															u("slotMinutes")
													)
											);
