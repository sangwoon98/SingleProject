...
(function () {
		...
		(window.CKEDITOR = (function () {
			var b = {
					...
					basePath: (function () {
						...
						if (!a)
							for (
								var b = document.getElementsByTagName("script"), h = 0;
								h < b.length;
								h++
							) {
								var c = b[h].src.match(
	...
	(function () {
		...
		CKEDITOR.tools = {
			...
			extend: function (a) {
				...
				for (var f = 1; f < b; f++) {
					var j = arguments[f],
						i;
					for (i in j)
						if (d === true || a[i] == void 0) if (!c || i in c) a[i] = j[i];
				}
				return a;
			},
				...
				for (var a = [].concat(a), b, d = [], c = 0; c < a.length; c++)
					if ((b = a[c]))
						/@import|[{}]/.test(b)
							...
							: d.push('<link type="text/css" rel=stylesheet href="' + b + '">');
				return d.join("");
			},
			...
			normalizeCssText: function (a, b) {
				var d = [],
					c,
					f = CKEDITOR.tools.parseCssText(a, true, b);
	...
	CKEDITOR.tools.extend(CKEDITOR.dom.node.prototype, {
		...
		getAddress: function (b) {
			for (
				var c = [], a = this.getDocument().$.documentElement, e = this.$;
				e && e != a;
		...
		getIndex: function (b) {
			...
			do
				if (
					!b ||
					!(c != this.$ && c.nodeType == CKEDITOR.NODE_TEXT && (e || !c.nodeValue))
				) {
					a++;
		...
		getAscendant: function (b, c) {
			...
			for (; a; ) {
				if (
					a.nodeName &&
					((e = a.nodeName.toLowerCase()), typeof b == "string" ? e == b : e in b)
				)
					return new CKEDITOR.dom.node(a);
				try {
					a = a.parentNode;
		...
		CKEDITOR.tools.extend(CKEDITOR.dom.element.prototype, {
			...
			isEmptyInlineRemoveable: function () {
				...
				for (var a = this.getChildren(), b = 0, d = a.count(); b < d; b++) {
					...
					if (
						...
						((c.type == CKEDITOR.NODE_ELEMENT && !c.isEmptyInlineRemoveable()) ||
							(c.type == CKEDITOR.NODE_TEXT && CKEDITOR.tools.trim(c.getText())))
					)
						return false;
				}
				return true;
			...
			hasAttributes:
				CKEDITOR.env.ie && (CKEDITOR.env.ie7Compat || CKEDITOR.env.ie6Compat)
					...
					: function () {
							...
							return (
								b > 0 && (b > 2 || !d[a[0].nodeName] || (b == 2 && !d[a[1].nodeName]))
							);
					  },
			...
			unselectable: function () {
				...
				if (CKEDITOR.env.ie || CKEDITOR.env.opera) {
					...
					for (
						var a, b = this.getElementsByTag("*"), c = 0, h = b.count();
						c < h;
						c++
					) {
						a = b.getItem(c);
			...
			scrollIntoParent: function (a, b, c) {
				...
				a instanceof CKEDITOR.dom.window &&
					(a = q ? j.getBody() : j.getDocumentElement());
				j = a.getWindow();
				f = n(this, j);
				var k = n(a, j),
					g = this.$.offsetHeight;
				...
				for (var c = this.$.attributes, b = b || {}, h = 0; h < c.length; h++) {
					...
					if (!(j in b))
						if (j == "checked" && (i = this.getAttribute(j))) a.setAttribute(j, i);
							...
							f.specified ||
							(CKEDITOR.env.ie && f.nodeValue && j == "value")
						) {
	...
	(function () {
		...
		CKEDITOR.dom.element.prototype.getBogus = function () {
			...
			return a &&
				(!CKEDITOR.env.ie ? a.is && a.is("br") : a.getText && e.test(a.getText()))
				? a
				: false;
		};
	})();
		...
		function b() {
			...
			return function (g) {
				...
				return (g.type == CKEDITOR.NODE_TEXT &&
					...
					(g.type == CKEDITOR.NODE_ELEMENT && !g.is(h))
					? false
					: true;
			};
		}
			...
			d = function (a, b, c, k) {
				...
				if (b == 2) {
					...
					if (
						r &&
						x &&
						(g.$.parentNode != r.$.parentNode || d.$.parentNode != x.$.parentNode)
					) {
						b = x.getIndex();
			...
			enlarge: function (a, b) {
				switch (a) {
									...
									if (m.type == CKEDITOR.NODE_TEXT) {
										...
										if (j && CKEDITOR.dtd.$removeEmpty[m.getName()]) {
											...
											if (/[^\s\ufeff]/.test(x)) m = null;
												...
												for (
													var l = m.$.getElementsByTagName("*"), s = 0, C;
													(C = l[s++]);
					...
					case CKEDITOR.ENLARGE_LIST_ITEM_CONTENTS:
						...
						var y,
							t,
							w = CKEDITOR.dom.walker.blockBoundary(
						...
						this.setStartAt(
							y,
							!y.is("br") && ((!f && this.checkStartOfBlock()) || (f && y.contains(f)))
						...
						this.setEndAt(
							y,
							(!f && this.checkEndOfBlock()) || (f && y.contains(f))
			...
			getNextParagraph: function (b) {
				...
				if (q)
					(i = d.getLast()) &&
						i.type == CKEDITOR.NODE_ELEMENT &&
						i.getName() == "br" &&
						(CKEDITOR.env.ie || i.getPrevious(e) || i.getNext(e)) &&
						i.remove();
				if (!this._.nextNode)
					this._.nextNode = l || d.equals(n) || !n ? null : c(d, 1, n);
				return d;
			},
		};
	...
	(function () {
		function b(a, b, g, f, e) {
			var h = b.name;
			if (
				(f || typeof a.elements != "function" || a.elements(h)) &&
				(!a.match || a.match(b))
			) {
				if ((f = !e)) {
					a: if (a.nothingRequired) f = true;
						...
						f =
							d(b.styles, a.requiredStyles) && d(b.attributes, a.requiredAttributes);
					}
					f = !f;
				}
				if (!f) {
		...
		function k(a) {
			return (
				a.type == CKEDITOR.NODE_ELEMENT && (a.name == "br" || A.$block[a.name])
			);
		}
		function g(a, b, c) {
			var g = a.name;
				...
				if (b == "br") {
					...
					b: {
						...
						for (var f = 0, e = g.length, h; f < e; ++f) {
							h = g[f];
							if (h.type == CKEDITOR.NODE_ELEMENT && !d[h.name]) {
								d = false;
								break b;
							}
						}
					...
					if (d) {
						...
						d = a.parent;
						for (
							var f = d.type == CKEDITOR.NODE_DOCUMENT_FRAGMENT || d.name == "body",
								p,
								e = g.length;
							e > 0;
							...
							if (
								f &&
								(h.type == CKEDITOR.NODE_TEXT ||
									(h.type == CKEDITOR.NODE_ELEMENT && A.$inline[h.name]))
							) {
								if (!p) {
		...
		CKEDITOR.filter = function (a) {
			...
			if (a instanceof CKEDITOR.editor) {
				...
				if (a === true) this.disabled = true;
					...
					(c == CKEDITOR.ENTER_DIV || d == CKEDITOR.ENTER_DIV) && g.push("div");
		...
		CKEDITOR.filter.prototype = {
			allow: function (b, c, g) {
				...
				for (h in b) {
					e = b[h];
					e =
						typeof e == "boolean" ? {} : typeof e == "function" ? { match: e } : r(e);
			...
			applyTo: function (a, b, c) {
				...
				d.length && (m = true);
				for (
					var o, t, a = [], c = ["p", "br", "div"][this.enterMode - 1];
					(o = d.pop());
				...
				for (; (t = a.pop()); ) {
					...
					if (o.parent)
						switch (t.check) {
							case "it":
								A.$removeEmpty[o.name] && !o.children.length
									? g(o, c, a)
									: n(o) || g(o, c, a);
								break;
							case "el-up":
								o.parent.type != CKEDITOR.NODE_DOCUMENT_FRAGMENT &&
									!A[o.parent.name][o.name] &&
									g(o, c, a);
								break;
							case "parent-down":
								o.parent.type != CKEDITOR.NODE_DOCUMENT_FRAGMENT &&
			...
			check: function (a, b, c) {
				...
				b =
					m.length > 0
						? false
						: CKEDITOR.tools.objectCompare(d.attributes, o.attributes, true)
						? true
						: false;
	...
	CKEDITOR.resourceManager.prototype = {
		...
		load: function (b, c, a) {
			...
			for (
				var e = this.loaded, d = this.registered, h = [], f = {}, j = {}, i = 0;
				i < b.length;
				i++
			) {
				var l = b[i];
										...
										if (o.icons)
											for (var k = o.icons.split(","), g = k.length; g--; )
												CKEDITOR.skin.addIcon(
													...
													o.path +
														"icons/" +
														(CKEDITOR.env.hidpi && o.hidpi ? "hidpi/" : "") +
														k[g] +
														".png"
												);
										c[j] = 1;
	...
	(function () {
		function b(b, e, h) {
			...
			if (e !== void 0) {
				...
				if (
					CKEDITOR.env.ie &&
					CKEDITOR.env.quirks &&
					h == CKEDITOR.ELEMENT_MODE_INLINE
				)
		...
		function n(a) {
			var b = a.config,
				...
				e = b.removePlugins;
			if (d)
				var f = RegExp("(?:^|,)(?:" + d.replace(/\s*,\s*/g, "|") + ")(?=,|$)", "g"),
					c = c.replace(f, ""),
					c = c + ("," + d);
			if (e)
				var h = RegExp("(?:^|,)(?:" + e.replace(/\s*,\s*/g, "|") + ")(?=,|$)", "g"),
					c = c.replace(h, "");
			...
			CKEDITOR.plugins.load(c.split(","), function (c) {
				...
				for (var k in c) {
					...
					if (m && !a.lang[k]) {
						...
						if (CKEDITOR.tools.indexOf(m, a.langCode) >= 0) i = a.langCode;
							...
							i =
								i != a.langCode && CKEDITOR.tools.indexOf(m, i) >= 0
									? i
									: CKEDITOR.tools.indexOf(m, "en") >= 0
									? "en"
									: m[0];
						}
						if (!o.langEntries || !o.langEntries[i])
				...
				CKEDITOR.scriptLoader.load(f, function () {
					...
					for (h = 0; h < a.config.blockedKeystrokes.length; h++)
						a.keystrokeHandler.blockedKeystrokes[a.config.blockedKeystrokes[h]] = 1;
					a.status = "loaded";
					a.fireOnce("loaded");
	...
	(function () {
		...
		CKEDITOR.htmlParser.prototype = {
			...
			parse: function (a) {
				...
				if (a.length > h) this.onText(a.substring(h, a.length));
			},
		};
	})();
							...
							if (!g.isUnknown && !u.isUnknown && !h[d])
								if (u.isOptionalClose) k.onTagClose(f);
									...
									((f = f[f.length - 1]) && f.name == "li") ||
										n((f = new CKEDITOR.htmlParser.element("li")), u);
	...
	(function () {
		...
		function k(a, b) {
			return a.replace(b, function (a, b, c) {
				a.indexOf("<textarea") === 0 &&
					(a = b + v(c).replace(/</g, "&lt;").replace(/>/g, "&gt;") + "</textarea>");
		...
		function p(a) {
			return a.replace(/<\!--(?!{cke_protected})[\s\S]+?--\>/g, function (a) {
				return (
					"<!--" + A + "{C}" + encodeURIComponent(a).replace(/--/g, "%2D%2D") + "-->"
				);
			});
		}
		function v(a) {
		...
		var E = /(?:&nbsp;|\xa0)$/,
			...
			C = {
				...
				attributes: {
					class: function (a) {
						return (
							CKEDITOR.tools.ltrim(a.replace(/(?:^|\s+)cke_[^\s]*/g, "")) || false
						);
					},
				},
			};
	...
	(function () {
		...
		CKEDITOR.inlineAll = function () {
			...
			for (a in CKEDITOR.dtd.$editable)
				for (
					var e = CKEDITOR.document.getElementsByTag(a), d = 0, h = e.count();
					d < h;
					d++
				) {
					b = e.getItem(d);
	...
	(function () {
		...
		CKEDITOR.editor.prototype.resize = function (a, b, c, e) {
			...
			l.setStyle(
				"height",
				Math.max(
					b - (c ? 0 : (e.$.offsetHeight || 0) - (l.$.clientHeight || 0)),
					0
				) + "px"
			);
			n && (n.style.width = "100%");
	...
	(function () {
		...
		function d(b) {
			...
			if (g.contains(e)) {
				...
				g = function (b, d) {
					return function (b, g) {
						g && b.type == CKEDITOR.NODE_ELEMENT && b.is(e) && (c = b);
		...
		CKEDITOR.editable = CKEDITOR.tools.createClass({
			...
			proto: {
				focus: function () {
					...
					CKEDITOR.env.safari &&
						!this.isInline() &&
						(CKEDITOR.document.getActive().equals(this.getWindow().getFrame()) ||
							this.getWindow().focus());
				},
				on: function (a, b) {
							...
							if (c in { 8: 1, 46: 1 }) {
								...
								if ((e = d(e))) {
									...
									if (
										...
										f.blockLimit &&
										f.blockLimit.is("td") &&
										(h = f.blockLimit.getAscendant("table")) &&
										b.checkBoundaryOfElement(h, c ? CKEDITOR.START : CKEDITOR.END) &&
		...
		var n = (function () {
					...
					if (b(g)) {
						...
						if (A && f in CKEDITOR.dtd.$list) k = k.concat(c(g, d));
							...
							if (f == "br" && g.data("cke-eol") && (!p || p == n - 1)) {
								E =
									(e = p ? k[p - 1].node : j.getItem(p + 1)) && (!b(e) || !e.is("br"));
			...
			function g(a) {
				return (
					a && b(a) && (a.is(i.$removeEmpty) || (a.is("a") && !a.isBlockBoundary()))
				);
			}
			function e(a, c, d, g) {
				var f = a.clone(),
			...
			return function (m, l, z) {
				...
				if (!C.checkReadOnly()) {
					var t =
							new CKEDITOR.dom.elementPath(C.startContainer, C.root).blockLimit ||
							C.root,
						l = {
							type: l,
							dontFilter: y,
					...
					for (
						;
						(D =
							b(s.startContainer) && s.startContainer.getChild(s.startOffset - 1)) &&
						b(D) &&
						D.isBlockBoundary() &&
						I.contains(D);
					...
					w = s.createBookmark();
					(t = w.startNode.getPrevious(a)) && b(t) && g(t) && y.push(t);
					...
					if (w) {
						...
						for (h(I, F); w < I.length; w++) {
							...
							if (B) y = w > 0;
								...
								if ((B = F.startPath().block) && !B.equals(H)) {
									if ((J = B.getBogus())) {
										J.remove();
										D.push(B);
									}
								...
								if (s && G.isElement) {
									...
									if (B) {
										...
										K = d(G.node, K);
										for (
											var M = [], R = K.length, T = 0, U = void 0, V = 0, W = -1;
											T < R;
											T++
										) {
											U = K[T];
					...
					if (!l.dontMoveCaret) {
						for (
							H = b(J.startContainer) && J.startContainer.getChild(J.startOffset - 1);
							H && b(H) && !H.is(i.$empty);
	...
	(function () {
		...
		var i,
			l,
			n = CKEDITOR.dom.walker.invisible(1);
		CKEDITOR.on("instanceCreated", function (a) {
			function d() {
			...
			e.on("contentDom", function () {
				...
				if (CKEDITOR.env.ie || CKEDITOR.env.opera || i) {
					...
					o
						? g.attachListener(g, "beforedeactivate", n, null, null, -1)
						: g.attachListener(e, "selectionCheck", n, null, null, -1);
					g.attachListener(
						g,
						"blur",
						function () {
				...
				var a = o
					? (function () {
							...
							var b = function (b, c) {
								...
								return { container: o, offset: t > 0 ? -e : o.nodeValue.length + e };
							};
							return function () {
					...
					if (b)
						for (var d = c.ranges, e = 0; e < d.length; e++) {
							...
							if (!f.collapsed) {
								...
								b &&
									b.type == CKEDITOR.NODE_TEXT &&
									(i >= b.getLength() ? l.setStartAfter(b) : l.setStartBefore(b));
								h &&
									h.type == CKEDITOR.NODE_TEXT &&
									(j ? l.setEndAfter(h) : l.setEndBefore(h));
					...
					c = CKEDITOR.tools.tryThese(
						...
						function () {
							for (
								...
								e &&
								(!(c = a.getEnclosedNode()) ||
									!(c.type == CKEDITOR.NODE_ELEMENT && q[c.getName()] && (d = c)));
								e--
							)
					...
					if (this.isLocked) {
						...
						if (o) {
							...
							((b.startContainer.type == CKEDITOR.NODE_ELEMENT &&
								b.startContainer.getName() in f) ||
								(b.endContainer.type == CKEDITOR.NODE_ELEMENT &&
									b.endContainer.getName() in f)) &&
							...
							for (d = 0; d < a.length; d++) {
								...
								if (
									...
									if (
										(!j && !c && i.is(CKEDITOR.dtd.$removeEmpty)) ||
		...
		if (!a) {
			...
			this.on("selectionChange", function (b) {
				for (var c = 0; c < a.length; c++) {
					var h = a[c],
						f = h.style.checkActive(b.data.path)
	...
	(function () {
						...
						if (
							...
							if (
								...
								if (
									...
									m ||
									(r == CKEDITOR.NODE_ELEMENT && !n.getChildCount())
								) {
									for (
										var r = n, x;
										(g = !r.getNext(C)) &&
		...
		function a(a) {
			...
			if (a.collapsed) {
				for (
					var d = new CKEDITOR.dom.elementPath(c.getParent(), a.root), e, f = 0, g;
		...
		function q(a) {
			for (
				var b = m(this), c = a.getElementsByTag(this.element), d = c.count();
				--d >= 0;
		...
		CKEDITOR.style.prototype = {
			...
			checkElementMatch: function (a, b) {
				...
				var d = a.getName();
				if (
					typeof this.element == "string" ? d == this.element : d in this.element
				) {
									...
									a: {
										...
										for (g in d)
											if (
												!(
													g in e &&
													(e[g] == d[g] || d[g] == "inherit" || e[g] == "inherit")
												)
											) {
												d = false;
												break a;
	...
	(function () {
		function b(a, b, c) {
			...
			b[c ? "endContainer" : "startContainer"] = f;
			return b;
		}
							...
							if (f)
								for (var l = 0; a[h + l + 1]; ) {
									for (
										var n = j.document,
											f = 0,
											i = n.getById(c[l].endNode),
											n = n.getById(c[l + 1].startNode);
										;
...
*/
		...
		var b = (function () {
				return function (b, e) {
					for (var a = b.match(/[^#]./g), c = 0; 3 > c; c++) {
						...
						d = parseInt(a[c], 16);
						d = (
							"0" + (0 > e ? 0 | (d * (1 + e)) : 0 | (d + (255 - d) * e)).toString(16)
						).slice(-2);
						f[h] = d;
					}
					return "#" + a.join("");
		...
		function J(a) {
			...
			f
				...
				: ((g = [
				  ...
				  CKEDITOR.env.ie6Compat &&
						((d =
							"<html><body style=\\'background-color:" + d + ";\\'></body></html>"),
						g.push(
			...
			CKEDITOR.ui.dialog = {
				uiElement: function (a, b, c, d, e, g, f) {
					if (!(4 > arguments.length)) {
						...
						for (
							var l = h["class"] && h["class"].split ? h["class"].split(" ") : [],
								i = 0;
							i < l.length;
							i++
						)
		...
		function m(a) {
			return {
				...
				exec: function (f) {
					...
					if (c && c.length) {
						for (
							var h = {}, e = d.createBookmarks(), c = c.createIterator(), g, j = 0;
							(g = c.getNextRange(1));
	...
	(function () {
		function w(a) {
			...
			function k(d) {
				...
				c.on(d, l);
				(7 < CKEDITOR.env.version ? b.$ : b.$.selection.createRange()).execCommand(
					d
				);
			...
			(function () {
				function b(c, e, d, l, i) {
					...
					a.addCommand(e, d);
					a.ui.addButton &&
						a.ui.addButton(c, { label: f, command: e, toolbar: "clipboard," + l });
					a.addMenuItems &&
						a.addMenuItem(e, { label: f, command: e, group: "clipboard", order: i });
				}
		...
		function y(a, b) {
			function e(a) {
				return (
					CKEDITOR.tools.repeat("</p><p>", ~~(a / 2)) + (1 == a % 2 ? "<br>" : "")
				);
			}
			b = b
				.replace(/\s+/g, " ")
			...
			render: function (a, b) {
				...
				var d = {
					...
					o = CKEDITOR.tools.addFunction(function (a) {
						if (d.onkey)
							return (
								(a = new CKEDITOR.dom.event(a)), !1 !== d.onkey(d, a.getKeystroke())
							);
					}),
			...
			proto: {
				...
				onKeyDown: function (a) {
					...
					switch (b) {
						case "next":
							for (
								var a = this._.focusIndex, b = this.element.getElementsByTag("a"), c;
								(c = b.getItem(++a));
		...
		CKEDITOR.ui.floatPanel = CKEDITOR.tools.createClass({
			...
			proto: {
						...
						function () {
							var a = CKEDITOR.tools.bind(function () {
								...
								m
									? h < c &&
									  (f =
											i > c ? f + c : b.width > c ? f - a.left : f - a.right + b.width)
									: h < c &&
									  (f =
											i > c ? f - c : b.width > c ? f - a.right + b.width : f - a.left);
								c = a.top;
								b.height - a.top < g &&
									(l =
										c > g ? l - g : b.height > g ? l - a.bottom + b.height : l - a.top);
								if (CKEDITOR.env.ie) {
				...
				hideChild: function (a) {
					...
					b &&
						(delete b.onHide, delete this._.activeChild, b.hide(), a && this.focus());
				},
			},
		});
		CKEDITOR.on("instanceDestroyed", function () {
			...
			function m(l, g, e, h) {
				...
				c.ui.add(l, CKEDITOR.UI_PANELBUTTON, {
					...
					onOpen: function () {
						...
						do
							b =
								(a && a.getComputedStyle("back" == g ? "background-color" : "color")) ||
								"transparent";
						...
						if ("?" == a) {
							var e = arguments.callee,
								d = function (a) {
									...
									"ok" == a.name &&
										e(this.getContentElement("picker", "selectedColor").getValue(), b);
								};
				...
				a.on("uiSpace", function (c) {
					if (c.data.space == a.config.toolbarLocation) {
						...
						a.config.toolbarCanCollapse &&
							a.elementMode != CKEDITOR.ELEMENT_MODE_INLINE &&
							b.push(
								'<span class="cke_toolbox_main"' + (d ? ">" : ' style="display:none">')
							);
	...
	(function () {
		...
		function l() {
			...
			return !a.getContentElement(this["for"][0], this["for"][1]).getInputElement()
				.$.value || !a.getContentElement(this["for"][0], this["for"][1]).getAction()
				? !1
				: !0;
		}
		function m(a, c, d) {
				...
				i = function (c) {
					...
					if (r) {
						...
						if (e) {
							...
							d.setStyle(
								h,
								s(("pin" == e ? u : p) + m + ("pin" == e ? 0 : "left" == h ? n : -n))
							);
					...
					for (
						...
						for (
							...
							var j =
								...
								m =
									this.state == CKEDITOR.TRISTATE_OFF && (!i || l(h, !0) != this.value);
							e
								? m
									? h.addClass(e)
		...
		function C(c, j, f) {
			...
			for (
				...
				if (
					...
					e.push(g), CKEDITOR.dom.element.setMarker(f, g, "list_item_processed", !0);
			}
			g = null;
					...
					if (o.indent == m) {
						...
						for (e = 0; e < x; e++) {
							...
							if (t.type == CKEDITOR.NODE_ELEMENT && t.isBlockBoundary()) {
								...
								w &&
									(l || ((l = h.createElement(p)), a && l.setAttribute("dir", n)),
	...
	(function () {
		...
		function r(a, b, d) {
			return (b = b[d ? "getPrevious" : "getNext"](function (b) {
				return (
					(b && b.type == CKEDITOR.NODE_TEXT && !z(b)) || (m(b) && !p(b) && !v(a, b))
				);
			}));
		}
		function da(a) {
				...
				place: function () {
					...
					d.is(x) || (k.top > b.scroll.y - 15 && k.top < b.scroll.y + 5)
						...
						? ((k.top = a.inInlineMode
								...
								: b.pane.bottom - 1),
						  this.look(y))
						: (a.inInlineMode && (k.top -= b.editable.top + b.editable.border.top),
						  this.look(s));
					a.inInlineMode &&
						(k.top--,
					...
					function () {
						...
						b.is(w.$inline) ||
							...
							b.attachListener(b, "keydown", function (b) {
								if ("wysiwyg" == a.mode)
									switch (
										((b = b.data.getKeystroke()), a.getSelection().getStartElement(), b)
									) {
										case 2228240:
										case 16:
							...
							a.on("loadSnapshot", function () {
								for (
									var b = a.document.getElementsByTag(c.enterBehavior), d, e = b.count();
									e--;
									...
									if ((d = b.getItem(e)).hasAttribute("data-cke-magicline-hot")) {
										c.hotNode = d;
										c.lastCmdDirection =
											"true" === d.getAttribute("data-cke-magicline-dir") ? !0 : !1;
										break;
									}
							}),
							(this.backdoor = {
				...
				if (a.elementMode != CKEDITOR.ELEMENT_MODE_INLINE) {
					...
					a.on(
						...
						function () {
							...
							b.setState(
								b.state == CKEDITOR.TRISTATE_DISABLED ? CKEDITOR.TRISTATE_DISABLED : l
							);
						},
						null,
						null,
			...
			init: function (a) {
				...
				a.on(
					...
					function (b) {
						...
						if (
							c &&
							(d || /(class=\"?Mso|style=\"[^\"]*\bmso\-|w:WordDocument)/.test(c))
						) {
			...
			i = {
				...
				exec: function (a) {
					...
					if (b.fullPage)
						...
						d &&
							(d.getAttribute("id") && (b += 'id="' + d.getAttribute("id") + '" '),
			...
			r = function () {
				...
				var b = this,
					f = function () {
						if (
							!(
								"undefined" != typeof d.instances[b.name] || null != d.instances[b.name]
							)
						) {
							var a = b.config,
								c = {};
				...
				e.equalsContent = CKEDITOR.tools.override(e.equalsContent, function (a) {
					return function (b) {
						var c = this.contents,
							f = b.contents,
			...
			init: function (a) {
				...
				c = function (b) {
					...
					CKEDITOR.env.opera || CKEDITOR.env.air
						...
						: p.setState(
								d.isScaytEnabled(a) ? CKEDITOR.TRISTATE_ON : CKEDITOR.TRISTATE_OFF
						  );
				};
		...
		g.prototype = {
			...
			lock: function () {
				if (this.locked) this.locked.level++;
					...
					this.locked = {
						update:
							this.currentImage && this.currentImage.equalsContent(a) ? a : null,
						level: 1,
					};
				}
			},
	...
	CKEDITOR.config.plugins =
		"dialogui,dialog,about,a11yhelp,dialogadvtab,basicstyles,bidi,blockquote,clipboard,button,panelbutton,panel,floatpanel,colorbutton,colordialog,templates,menu,contextmenu,div,resize,toolbar,elementspath,enterkey,entities,popup,filebrowser,find,fakeobjects,flash,floatingspace,listblock,richcombo,font,forms,format,htmlwriter,horizontalrule,iframe,wysiwygarea,image,indent,indentblock,indentlist,smiley,justify,link,list,liststyle,magicline,maximize,newpage,pagebreak,pastetext,pastefromword,preview,print,removeformat,save,selectall,showblocks,showborders,sourcearea,specialchar,menubutton,scayt,stylescombo,tab,table,tabletools,undo,wsc";
