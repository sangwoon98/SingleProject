...
(function () {
	function y(c) {
		return (
			c.type == CKEDITOR.NODE_TEXT && 0 < c.getLength() && (!o || !c.isReadOnly())
		);
	}
	function s(c) {
		return !(
		...
		n = function (c, g) {
			...
			return {
								...
								children: [
									...
									},
									{
										...
										onClick: function () {
											...
											e.replaceCounter
												? (alert(f.replaceSuccessMsg.replace(/%1/, e.replaceCounter)),
												  c.fire("saveSnapshot"))
												: alert(f.notFoundMsg);
										},
									},
				...
				onShow: function () {
					...
					b.select();
					this.selectPage(g);
					this[("find" == g && this._.editor.readOnly ? "hide" : "show") + "Page"](
						"replace"
					);
				},
				onHide: function () {
