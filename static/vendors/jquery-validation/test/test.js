	...
	var v = $("#form").validate({
		...
		showErrors: function () {
			container
				.find("h3")
				.html(
					jQuery.validator.format("There are {0} errors in your form.", this.size())
				);
			this.defaultShowErrors();
		},
	});
...
test("error containers, with labelcontainer I", function () {
	...
	equal(
		0,
		labelcontainer.find("label").length,
...
test("errorcontainer, show/hide only on submit", function () {
	...
	$("#testForm1").triggerHandler("keyup", [
		jQuery.event.fix({ type: "keyup", target: $("#firstname")[0] }),
	]);
