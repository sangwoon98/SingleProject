	...
	tinymce.create("tinymce.util.URI", {
		URI: function (u, s) {
			...
			if (u.indexOf("/") === 0 && u.indexOf("//") !== 0)
				u =
					(s.base_uri ? s.base_uri.protocol || "http" : "http") + "://mce_host" + u;
								...
								if (s.url_converter) {
									u = u.replace(/url\([\'\"]?([^\)\'\"]+)\)/g, function (x, c) {
										return (
											"url(" +
											t.encode(
												s.url_converter.call(s.url_converter_scope || t, t.decode(c), b, n)
											) +
											")"
										);
									});
		...
		_postProcess: function (o) {
			...
			if (o.format == "html") {
				...
				if (!o.set) {
					h = h.replace(
						/<p>\s+<\/p>|<p([^>]+)>\s+<\/p>/g,
						s.entity_encoding == "numeric" ? "<p$1>&#160;</p>" : "<p$1>&nbsp;</p>"
					);
	...
	tinymce.create("tinymce.Editor", {
		...
		init: function () {
			...
			if (t.theme.init && s.init_theme)
				t.theme.init(
					t,
					ThemeManager.urls[s.theme] || tinymce.documentBaseURL.replace(/\/$/, "")
				);
...
(function () {
	...
	tinymce.create("tinymce.ForceBlocks", {
		...
		insertPara: function (e) {
			...
			function first(n) {
				return (
					d.createTreeWalker(n, NodeFilter.SHOW_TEXT, null, false).nextNode() || n
				);
			}
			r = d.createRange();
...
(function () {
	...
	tinymce.create("tinymce.ControlManager", {
		...
		createListBox: function (id, s, cc) {
			...
			s = extend(
				{ title: s.title, class: "mce_" + id, scope: s.scope, control_manager: t },
				s
			);
			id = t.prefix + id;
